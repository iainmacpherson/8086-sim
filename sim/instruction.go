...
type Instruction struct {
	...
	Disassembly string               // the disassembled instruction
}
...
func (instr *Instruction) calculateEffectiveAddress() string {
	memory := "["
	address := AddressingRegs[instr.RM]
	displacement := fmt.Sprint(uint16(instr.DISP_HI)<<8 | uint16(instr.DISP_LO))

	if instr.MOD == 0b00 {
		if instr.RM == 0b110 { // when direct addressing we only use the displacement
			memory += displacement
		} else {
			memory += address
		}
	} else {
		// addressing using the different displacements
		memory += address
		memory += " + "
		memory += displacement
	}
